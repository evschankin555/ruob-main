						...
						<?foreach ($arFields as $FIELD):?>
							...
							<?}?>
										...
										<?switch ($FIELD){
											...
											<?break;?>
											<?case "PERSONAL_PHONE":?>
												<input size="30" inputmode="tel" type="text" id="input_<?=$FIELD;?>" name="REGISTER[<?=$FIELD?>]" class="phone_input <?=(array_key_exists( $FIELD, $arResult["ERRORS"] ))? 'error': ''?>" <?=($arResult["REQUIRED_FIELDS_FLAGS"][$FIELD] == "Y" ? "required": "");?> value="<?=$arResult["VALUES"][$FIELD]?>" />
											<?break;?>
											<?break;
											default:?>
												<?// hide login?>
