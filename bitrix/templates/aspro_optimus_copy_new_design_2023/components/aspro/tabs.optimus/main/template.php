...
$this->setFrameMode(true);
	...
	if($arResult["SHOW_SLIDER_PROP"]){?>
		<div class="tab_slider_wrapp specials <?=$class_block;?> best_block clearfix ajax_load">
			<?$arParams['SET_TITLE'] = 'N';
			$arParams["FILTER_HIT_PROP"] = array_keys($arResult["TABS"]);
			...
			<ul class="tabs_content" data-url="<?=$url?>">
				<?foreach($arResult["TABS"] as $code=>$title){?>
					...
					<li class="tab <?=$code?>_wrapp <?=($j++ ==1 ? "cur" : "");?>" data-code="<?=$code?>" data-col="<?=$col;?>"  data-filter="<?=($GLOBALS[$arParams["FILTER_NAME"]] ? urlencode(serialize($GLOBALS[$arParams["FILTER_NAME"]])) : '');?>">
