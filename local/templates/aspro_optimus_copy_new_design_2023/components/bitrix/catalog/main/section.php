...
if($section){
	...
	if(strlen($section["UF_SEO_TITLE"]))
		$arSection["UF_SEO_TITLE"] = $section["UF_SEO_TITLE"];
	if(strlen($section["UF_SEO_DESCRIPTION"]))
		$arSection["UF_SEO_DESCRIPTION"] = $section["UF_SEO_DESCRIPTION"];
...
<?if($itemsCnt):?>
			...
			<?if(!$arSeoItem):?>
				...
				<?if($arParams["SECTION_PREVIEW_DESCRIPTION"] != 'N' && strpos($_SERVER['REQUEST_URI'], 'PAGEN') === false):?>
					<?if($posSectionDescr=="BOTH"):?>
						<?if(strlen($APPLICATION->wdSeoUrlData['DESCRIPTION_TOP'])):?>
							<div class="group_description_block top">
								<div><?=$APPLICATION->wdSeoUrlData['DESCRIPTION_TOP']?></div>
							</div>
						<?elseif ($arSection[$section_pos_top]):?>
							<div class="group_description_block top">
    ...
    // Проверяем наличие параметра tags в URL
    if (isset($_GET['tags'])) {
        // URL-декодируем полученный тег
        $selectedTag = urldecode($_GET['tags']);
        // Создаем фильтр для выборки элементов с данным тегом
        $GLOBALS['arrFilter'] = array(
            'TAGS' => $selectedTag // Укажите здесь код свойства "Теги"
        );

        // Устанавливаем имя глобального фильтра в параметрах компонента
	...
	<?endif;?>
				...
				<?if($isAjax=="N"){?>
					<?if(!$arSeoItem):?>
						<?if($arParams["SECTION_PREVIEW_DESCRIPTION"] != 'N' && strpos($_SERVER['REQUEST_URI'], 'PAGEN') === false):?>
							<?if($posSectionDescr=="BOTH"):?>
								<?if(strlen($APPLICATION->wdSeoUrlData['DESCRIPTION_BOTTOM'])):?>
									<div class="group_description_block bottom">
										<div><?=$APPLICATION->wdSeoUrlData['DESCRIPTION_BOTTOM']?></div>
									</div>
...
if($arSeoItem)
{
	...
	?>
<?}
if($arSection["UF_SEO_TITLE"])
	$APPLICATION->SetPageProperty("title", $arSection["UF_SEO_TITLE"]);

if($arSection["UF_SEO_DESCRIPTION"])
