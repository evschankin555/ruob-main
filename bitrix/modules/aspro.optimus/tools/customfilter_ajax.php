...
if($action){
	if($action === 'init' || $action === 'save'){
		...
		AddEventHandler('catalog', 'OnCondCatControlBuildList', array('\Aspro\Optimus\Property\CustomFilter\CondCtrl', 'GetControlDescr'));
		...
		if($action === 'get_crosssales_iblockprops'){
			...
			if(check_bitrix_sessid() && $request->isPost()){
				...
				if($propertyId > 0){
					...
					if($property){
						$arExcludeUserTypes = \Aspro\Optimus\Property\CustomFilter\CondCtrl::getCrossSalesExcludePropertyUserTypes();
