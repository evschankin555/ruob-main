...
static function CIBlockElement_GetList($arOrder = array("SORT" => "ASC", "CACHE" => array("MULTI" => "Y", "GROUP" => array(), "RESULT" => array(), "TAG" => "", "PATH" => "", "TIME" => 36000000, "URL_TEMPLATE" => "")), $arFilter = array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array()){
				...
				if($arGroupBy === array()){
					...
					if($bGetDetailPageUrlsArray){
						...
						foreach($arByIBlock as $IBLOCK_ID => $arIB){
							...
							if($arSectionIDs){
								$arSections = COptimusCache::CIBlockSection_GetList(array("CACHE" => array("TAG" => COptimusCache::GetIBlockCacheTag($IBLOCK_ID), "MULTI" => "N", "GROUP" => array("ID"))), array("ID" => $arSectionIDs), false, array("ID", "CODE", "EXTERNAL_ID", "IBLOCK_ID"));
							}
							foreach($arIB as $SECTION_ID => $arIDs){
								if($arIDs){
									$rsElements = CIBlockElement::GetList(array(), array("ID" => $arIDs), false, false, array("ID", "DETAIL_PAGE_URL", "CANONICAL_PAGE_URL"));
									$rsElements->SetUrlTemplates(COptimusCache::$arIBlocksInfo[$IBLOCK_ID]["DETAIL_PAGE_URL"]);
						...
						foreach($arRes as $i => $arItem){
							if(count((array)$arNewDetailPageUrls[$arItem["ID"]]) > 1){
...
static function CIBlockSection_GetList($arOrder = array("SORT" => "ASC", "CACHE" => array("MULTI" => "Y", "GROUP" => array(), "RESULT" => array(), "TAG" => "", "PATH" => "", "TIME" => 36000000, "URL_TEMPLATE" => "")), $arFilter = array(), $bIncCnt = false, $arSelectFields = array(), $arNavStartParams = false){
...
static function CSaleBasket_GetList($arOrder = array("SORT" => "ASC"), $arFilter = array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array(), $cacheTag = "", $cacheTime = 36000000, $cachePath = ""){
			CModule::IncludeModule('sale');
			if(!strlen($cacheTag)){
				$cacheTag = "_notag";
			}
			if(!strlen($cachePath)){
				$cachePath = "/COptimusCache/sale/CSaleBasket_GetList/".$cacheTag."/";
			}
			$obCache = new CPHPCache();
...
static function CCatalogStore_GetList($arOrder = array("SORT" => "ASC"), $arFilter = array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array(), $cacheTag = "", $cacheTime = 36000000, $cachePath = ""){
			CModule::IncludeModule('catalog');
			if(!strlen($cacheTag)){
				$cacheTag = "_notag";
			}
			if(!strlen($cachePath)){
				$cachePath = "/COptimusCache/catalog/CCatalogStore_GetList/".$cacheTag."/";
			}
			$obCache = new CPHPCache();
...
static function CIBlockSection_GetCount($arOrder = array("CACHE" => array("MULTI" => "Y", "GROUP" => array(), "RESULT" => array(), "TAG" => "", "PATH" => "", "TIME" => 36000000)), $arFilter = array()){
			list($cacheTag, $cachePath, $cacheTime) = self::_InitCacheParams("iblock", __FUNCTION__, $arOrder["CACHE"]);
...
static function CIBlockElement_GetProperty($iblockID, $elementID, $arOrder = array("SORT" => "ASC", "CACHE" => array("MULTI" => "Y", "GROUP" => array(), "RESULT" => array(), "TAG" => "", "PATH" => "", "TIME" => 36000000)), $arFilter = array()){
			list($cacheTag, $cachePath, $cacheTime) = self::_InitCacheParams("iblock", __FUNCTION__, $arOrder["CACHE"]);
...
static function CForm_GetList(&$by = array('CACHE' => array('MULTI' => 'Y', 'GROUP' => array(), 'RESULT' => array(), 'TAG' => '', 'PATH' => '', 'TIME' => 36000000)), &$order = 'asc', $arFilter = array(), &$is_filtered, $min_permission = 10){
...
static function CForumMessage_GetListEx($arOrder = array("SORT" => "ASC"), $arFilter = array(), $arGroupBy = false, $iNum = 0, $arSelectFields = array(), $cacheTag = "", $cacheTime = 36000000, $cachePath = ""){
			CModule::IncludeModule('forum');
			if(!strlen($cacheTag)){
				$cacheTag = "_notag";
			}
			if(!strlen($cachePath)){
				$cachePath = "/COptimusCache/forum/CForumMessage_GetListEx/".$cacheTag."/";
			}
			$obCache = new CPHPCache();
		...
		public static function _MakeResultTreeArray($arParams, &$arItem, &$arItemResval, &$to){
...
static function GroupArrayBy($arItems, $arParams){
		...
		public static function _InitCacheParams($moduleName, $functionName, $arCache){
			...
			if(!strlen($cachePath)){
				$cachePath = "/COptimusCache/".$moduleName."/".$functionName."/".$cacheTag."/";
			}
			return array($cacheTag, $cachePath, $cacheTime);
		}

		public static function _GetElementSectionsArray($ID){
			$arSections = array();
		...
		public static function _GetFieldsAndProps($dbRes, $arSelectFields, $bIsIblockElement = false){
...
if($arIBlocksInfo[$IBLOCK_ID]["CODE"])
				return @COptimusCache::$arIBlocksInfo[$IBLOCK_ID]["CODE"].$IBLOCK_ID;
			}
		}
...
static function ClearTagIBlock($arFields){
			global $CACHE_MANAGER;
...
static function ClearTagByUser($arFields){
			if($arFields["ID"])
				self::ClearCacheByTag(self::GetUserCacheTag($arFields["ID"]));
		}
static function ClearTagIBlockBeforeDelete($ID){
			global $CACHE_MANAGER;
			$CACHE_MANAGER->ClearByTag("iblocks");
		}
static function ClearTagIBlockElement($arFields){
			global $CACHE_MANAGER;
			if($arFields["IBLOCK_ID"]){
				$CACHE_MANAGER->ClearByTag(COptimusCache::GetIBlockCacheTag($arFields["IBLOCK_ID"]));
			}
		}
static function ClearTagIBlockSection($arFields){
			global $CACHE_MANAGER;
			if($arFields["IBLOCK_ID"]){
				$CACHE_MANAGER->ClearByTag(COptimusCache::GetIBlockCacheTag($arFields["IBLOCK_ID"]));
			}
		}
static function ClearTagIBlockSectionBeforeDelete($ID){
			global $CACHE_MANAGER;
			if($ID > 0){
				if($IBLOCK_ID = COptimusCache::CIBlockSection_GetList(array("CACHE" => array("MULTI" => "N")), array("ID" => $ID), false, array("IBLOCK_ID"), true)){
					$CACHE_MANAGER->ClearByTag(COptimusCache::GetIBlockCacheTag($IBLOCK_ID));
				}
			}
		}
	}
	...
	/*AddEventHandler("iblock", "OnAfterIBlockAdd", Array("COptimusCache", "ClearTagIBlock"));
	AddEventHandler("iblock", "OnAfterIBlockUpdate", Array("COptimusCache", "ClearTagIBlock"));
	AddEventHandler("iblock", "OnBeforeIBlockDelete", Array("COptimusCache", "ClearTagIBlockBeforeDelete"));

	AddEventHandler("iblock", "OnAfterIBlockElementAdd", array("COptimusCache", "ClearTagIBlockElement"));
	AddEventHandler("iblock", "OnAfterIBlockElementUpdate", array("COptimusCache", "ClearTagIBlockElement"));
	AddEventHandler("iblock", "OnAfterIBlockElementDelete", array("COptimusCache", "ClearTagIBlockElement"));

	AddEventHandler("iblock", "OnAfterIBlockSectionAdd", Array("COptimusCache", "ClearTagIBlockSection"));
	AddEventHandler("iblock", "OnAfterIBlockSectionUpdate", Array("COptimusCache", "ClearTagIBlockSection"));
	AddEventHandler("iblock", "OnBeforeIBlockSectionDelete", Array("COptimusCache", "ClearTagIBlockSectionBeforeDelete"));
