...
$filter = ['IBLOCK_ID' => 14,'ACTIVE' => 'Y','NAME' => "%".$_REQUEST['q']."%"];
$rs = CIblockSection::GetList([],$filter,false,false,$select);
...
<?if(!empty($arResult["CATEGORIES"])):?>
	<table class="title-search-result">
        <?if($sections):?>
            <tr><td class="title-search-all-sections" colspan="<?=($arParams["SHOW_PREVIEW"]=="Y") ? '3' : '2'?>" >
                    <?foreach($sections as $section):?>
                        <a class="section_link" href="<?=$section['SECTION_PAGE_URL']?>"><?=$section['NAME']?></a>
                    <?endforeach;?>
                </td>
            </tr>
        <?endif;?>
