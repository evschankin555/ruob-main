...
if($arResult["ITEMS"]){?>
	...
	<?$bActiveFilter = \Aspro\Functions\CAsproOptimus::checkActiveFilterPage($arParams["SEF_RULE_FILTER"]);?>
			...
			<form name="<?echo $arResult["FILTER_NAME"]."_form"?>" action="<?echo $arResult["FORM_ACTION"]?>" method="get" class="smartfilter">
				<div class="bx_filter_parameters_box title">
					<div class="bx_filter_parameters_box_title filter_title <?=($bActiveFilter && $bActiveFilter[1] != 'clear' ? 'active-filter' : '')?>">
						<?
						$options = array(
							'PATH' => SITE_TEMPLATE_PATH.'/images/svg/catalog/filter.svg',
							'CLASS' => "icon",
							'INLINE' => false,
						);
						print_r( COptimus::showIconSvg($options) );
						?>
					...
					foreach($arResult["ITEMS"] as $key => $arItem){
					    if(isset($arItem["ASPRO_FILTER_SORT"]) && $arItem["VALUES"]){
							...
							<div class="bx_filter_parameters_box bx_sort_filter <?=$class;?>" data-expanded="<?=($arItem["DISPLAY_EXPANDED"] ? $arItem["DISPLAY_EXPANDED"] : "N");?>" data-prop_code="<?=strtolower($arItem["CODE"]);?>" data-property_id="<?=$arItem["ID"]?>">
								...
								<div class="bx_filter_parameters_box_title"><div><?=$arItem["NAME"]?></div>
								<?
								$options = array(
									'PATH' => SITE_TEMPLATE_PATH.'/images/svg/trianglearrow_down.svg',
									'CLASS' => "down colored_theme_hover_bg-el",
									'INLINE' => false,
								);
								print_r( COptimus::showIconSvg($options) );
								?>
					...
					<div class="bx_filter_parameters_box prices<?=(isset($arResult['PRICE_SET']) && $arResult['PRICE_SET'] == 'Y' ? ' set' : '');?>">
						...
						<div class="bx_filter_parameters_box_title title rounded3 box-shadow-sm" >
							...
							<span class="delete_filter colored_theme_bg_hovered_hover">
								<?
								$options = array(
									'PATH' => SITE_TEMPLATE_PATH.'/images/svg/catalog/cancelfilter.svg',
									'CLASS' => "delete_filter",
									'SHOW_WRAPPER' => false,
									'INLINE' => false,
								);
								...
								if(isset($arItem["PRICE"])):
									...
									?>
										<div class="price_block swipeignore">
											<?$bPriceSingle = count($arParams['PRICE_CODE']) < 2;?>
											<div class="bx_filter_parameters_box_title rounded3 prices <?=$bPriceSingle ? 'single' : ''?>"><?=($bPriceSingle ? Loc::getMessage("PRICE") : $arItem["NAME"]);?></div>
				...
				<?if($isFilter):?>
					...
					<button class="bx_filter_search_reset btn-link-text font_sxs colored_theme_text_with_hover<?=($bActiveFilter && $bActiveFilter[1] != 'clear' ? '' : ' hidden');?>" type="reset" id="del_filter" name="del_filter" data-href="">
						<?
						$options = array(
							'PATH' => SITE_TEMPLATE_PATH.'/images/svg/catalog/cleanfilters.svg',
							'CLASS' => "clear",
							'INLINE' => false,
						);
						print_r( COptimus::showIconSvg($options) );
						...
						<div class="bx_filter_block">
							<div class="bx_filter_parameters_box_container clearfix">
								...
								<button class="bx_filter_search_reset btn btn-default btn-transparent round-ignore<?=($bActiveFilter && $bActiveFilter[1] != 'clear' ? '' : ' hidden');?> pull-left" type="reset" id="del_filter1" name="del_filter1" data-href="" title="<?=Loc::getMessage("CT_BCSF_DEL_FILTER")?>">
									<?
									$options = array(
										'PATH' => SITE_TEMPLATE_PATH.'/images/svg/catalog/cleanfilters_big.svg',
										'CLASS' => "reset",
										'INLINE' => false,
									);
									print_r( COptimus::showIconSvg($options) );
