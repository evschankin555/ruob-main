...
if($detect->isMobile() || $detect->isTablet()){
	$arParams["DEFAULT_LIST_TEMPLATE"] = "list";
}
...
<div class="sort_header view_<?=$display?>">
	<!--noindex-->
		<div class="sort_filter <?=$bMobileCompactFilter ? 'compact_mobile_filter' : ''?>">
			...
			$sort = "SHOWS";
						$arParams["ELEMENT_SORT_FIELD"] = "PRICE";
		...
		<div class="sort__view">
			<?
            $arr = [
                'block' => '<svg class="sort__view-gridfour"><use xlink:href="#grid-four"></use></svg>',
                'list' => '<svg class="sort__view-gridrow"><use xlink:href="#view-row"></use></svg>'
            ];
            ...
            if($displayType == 'table'){
				...
				$url = str_replace('+', '%2B', $current_url);
				?>
				<a rel="nofollow" href="<?=$url;?>" class="sort__view-link sort_btn <?=$displayType?> <?=($display == $displayType ? 'current' : '')?>"><?=$arr[$displayType]?></a>
			<?endforeach;?>
		</div>
	<!--/noindex-->
</div>
<?if($sort == 'CATALOG_PRICE_1') {
	$sort_order .= ",nulls";
}?>