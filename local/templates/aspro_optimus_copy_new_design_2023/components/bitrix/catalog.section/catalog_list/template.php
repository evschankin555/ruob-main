							...
							<div class="wrapp_stockers">
								...
								<?=$arQuantityData["HTML"];?>
									<?if(isset($arItem['ARTICLE']) && $arItem['ARTICLE']['VALUE']){?>
                                        <div class="card__articul card__articul--products"><?=$arItem['ARTICLE']['NAME'];?>: <span><?=$arItem['ARTICLE']['VALUE'];?></span></div>
									<?}?>
							</div>
							<?$boolShowOfferProps = ($arItem['OFFERS_PROPS_DISPLAY']);
							$boolShowProductProps = (isset($arItem['DISPLAY_PROPERTIES']) && !empty($arItem['DISPLAY_PROPERTIES']));?>
							<?if($boolShowProductProps || $boolShowOfferProps):?>
                                <div class="list-param__wrap">
                                    <ul class="list-param products-row__list-param">
                                    ...
                                    <button class="list-param__wrap-btn js-list-param" type="button">Характеристики <span><svg><use xlink:href="#angle-down"></use></svg></span></button>
                                </div>
							<?endif;?>
						</div>
								...
								<?if($arParams["DISPLAY_WISH_BUTTONS"] != "N"):?>
									<?if(!$arItem["OFFERS"]):?>
										<div class="wish_item_button">
											<span class="wish_item to" data-item="<?=$arItem["ID"]?>" data-iblock="<?=$arItem["IBLOCK_ID"]?>">
                                                <svg class="heart-btn"><use xlink:href="#heart"></use></svg>
                                                <?php /*<div class="text-icons"><?=GetMessage('CATALOG_WISH')?></div> */ ?>
                                            </span>
											<span class="wish_item in added" style="display: none;" data-item="<?=$arItem["ID"]?>"
                                                  data-iblock="<?=$arItem["IBLOCK_ID"]?>">
                                            <svg class="heart-btn"><use xlink:href="#heart"></use></svg>
                                                <?php /*<div class="text-icons"><?=GetMessage('CATALOG_WISH_OUT')?></div>*/ ?>
                                            </span>
										</div>
									<?elseif($arItem["OFFERS"] && !empty($arItem['OFFERS_PROP'])):?>
										<div class="wish_item_button">
											<span class="wish_item to <?=$arParams["TYPE_SKU"];?>" data-item=""
                                                  data-iblock="<?=$arItem["IBLOCK_ID"]?>" data-offers="Y" data-props="<?=$arOfferProps?>">
                                            <svg class="heart-btn"><use xlink:href="#heart"></use></svg>
                                                <?php /*<div class="text-icons"><?=GetMessage('CATALOG_WISH')?></div>*/ ?>
                                            </span>
											<span class="wish_item in added <?=$arParams["TYPE_SKU"];?>" style="display: none;"
								...
								<?if($arParams["DISPLAY_COMPARE"] == "Y"):?>
									<?if(!$arItem["OFFERS"] || ($arParams["TYPE_SKU"] !== 'TYPE_1' || ($arParams["TYPE_SKU"] == 'TYPE_1' && !$arItem["OFFERS_PROP"]))):?>
										<div class="compare_item_button">
											<span class="compare_item to" data-iblock="<?=$arParams["IBLOCK_ID"]?>" data-item="<?=$arItem["ID"]?>" >
                                                <svg class="chart-btn"><use xlink:href="#chart3"></use></svg>
                                                <?php /*<div class="text-icons"><?=GetMessage('CATALOG_COMPARE')?></div>*/ ?>
                                            </span>
											<span class="compare_item in added" style="display: none;" data-iblock="<?=$arParams["IBLOCK_ID"]?>"
                                                  data-item="<?=$arItem["ID"]?>">
                                                <svg class="chart-btn"><use xlink:href="#chart3"></use></svg>
                                                <?php /*<div class="text-icons"><?=GetMessage('CATALOG_COMPARE_OUT')?></div>*/ ?>
                                            </span>
										</div>
									<?elseif($arItem["OFFERS"]):?>
										<div class="compare_item_button">
											<span class="compare_item to <?=$arParams["TYPE_SKU"];?>" data-iblock="<?=$arParams["IBLOCK_ID"]?>" data-item="" >
                                                <svg class="chart-btn"><use xlink:href="#chart3"></use></svg>
                                                <?php /*<div class="text-icons"><?=GetMessage('CATALOG_COMPARE')?></div>*/ ?>
                                            </span>
						...
						<div class="description_wrapp mobile">
							<div class="information inner_content js_offers__<?=$arItem['ID'];?>_list">
								...
								<div class="cost prices clearfix">
                                    ...
                                    <?=COptimus::showPriceMatrix($arItem, $arParams, $strMeasure, $arAddToBasketData);?>
                                            ...
                                            else
                                            {
                                                ...
                                                ?>₽" data-title="<?=
                                                htmlentities($arItem["NAME"])
                                                ?>" data-link="popup-price"><use xlink:href="#info"></use></svg>
                                                <?php endif;?>
								...
								<?elseif($arItem["OFFERS"]):?>
									...
									<?}?>
								<?endif;?>
<?if($arItem['PRICES']['BASE']['VALUE']<1):?>
<pre class="" style="display:none"><?print_r($arItem['PRICES']['BASE']['VALUE'])?></pre>
<span class="to-analog button transition_bg transparent" data-name="<?=$arItem['NAME']?>" data-item="<?=$arItem['ID']?>"><i></i><span>Подобрать аналог</span></span>
<?endif;?>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        var elements = document.querySelectorAll('.item-stock > span');

        elements.forEach(function(element) {
            if (element.textContent.includes('Заказать')) {
                element.style.color = '#888';
            }
        });
        // Добавление SVG к элементам .counter_block .plus
        var plusElements = document.querySelectorAll('.counter_block .plus');
        plusElements.forEach(function(el) {
            el.innerHTML = '<svg class="svg-plus"><use xlink:href="#plus"></use></svg>';
        });

        // Добавление класса minus-icon к элементам .counter_block .minus
        var minusElements = document.querySelectorAll('.counter_block .minus');
        minusElements.forEach(function(el) {
            el.classList.add('minus-icon');
        });

        // Обновляем данные модального окна по кредиту и тп
        var productsInfo = document.querySelectorAll('.products-row__price-info');
        productsInfo.forEach(function(el) {
            $(el).click(function(){
                let title = $(this).attr('data-title');
                let credit = $(this).attr('data-credit');
                let price = $(this).attr('data-price');

                document.getElementById('popup__price-title').innerHTML = title;
                document.getElementById('popup__price-credit').innerHTML = credit;
                document.getElementById('popup__price-price').innerHTML = price;
            });
        });
