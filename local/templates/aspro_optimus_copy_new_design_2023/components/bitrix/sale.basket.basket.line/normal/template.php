...
<div class="basket_normal cart <?=(!$count || $isBasketPage || $_POST["ACTION"]=='top' ? ' empty_cart ' : '')?> <?=(!$count && $isBasketPage ? 'ecart' : '');?> <?=($isBasketPage || $_POST["ACTION"]=='top' ? 'bcart' : '');?>">
		<div class="wraps_icon_block delay <?=($delayCount ? 'fill' : '' );?>">
			<a href="<?=$arParams["PATH_TO_BASKET"];?>#delayed" class="link" title="<?=GetMessage("BASKET_DELAY_LIST");?>"></a>
			<?if($delayCount){?>
				<div class="count">
					<span>
						<div class="items">
				...
				<div class="basket_popup_wrapper">
					<div class="basket_popup_wrapp" <?=($_SERVER["REQUEST_METHOD"] == "POST" && $_POST["ACTION"]=='del' && $_POST["ACTION"]!='top' ? "style='display: block;'" : "");?>>
						...
						<div class="total_wrapp clearfix">
							...
							<div class="but_row1">
								<?$bShowFastBuy = (\Bitrix\Main\Config\Option::get('aspro.optimus', 'SHOW_ONECLICKBUY_ON_BASKET_PAGE', 'Y') === 'Y')?>
								<a href="<?=$arParams["PATH_TO_BASKET"]?>" class="button<?=($bShowFastBuy ? ' transparent' : '');?> short"><span class="text"><?=GetMessage("GO_TO_BASKET");?></span></a>
								<?if ($bShowFastBuy):?>
									<span class="oneclickbuy button short fast_order" onclick="oneClickBuyBasket()"><?=GetMessage('QUICK_ORDER')?></span>
								<?endif;?>
							</div>
						</div>
