...
static function GetSKUPropsArray(&$arSkuProps, $iblock_id=0, $type_view="list", $hide_title_props="N", $group_iblock_id="N", $arItem = array(), $offerShowPreviewPictureProps = array()){
		$arSkuTemplate = array();
            ...
            )
            {
                if ('MORE_PHOTO' == $propertyCode && isset($item['MORE_PHOTO']) && !empty($item['MORE_PHOTO']))
                {
                    ...
                    foreach ($item['MORE_PHOTO'] as &$onePhoto)
                    {
                        ...
                        if($item['IPROPERTY_VALUES']['ELEMENT_DETAIL_PICTURE_FILE_ALT'])
                            $onePhoto['DESCRIPTION'] = $item['IPROPERTY_VALUES']['ELEMENT_DETAIL_PICTURE_FILE_ALT'];
                        $result[] = array(
                            'ID' => (int)$onePhoto['ID'],
                ...
                else
                {
                    ...
                    )
                    {
                        ...
                        {
                            if($item['IPROPERTY_VALUES']['ELEMENT_DETAIL_PICTURE_FILE_ALT'])
                                $oneFileValue['DESCRIPTION'] = $item['IPROPERTY_VALUES']['ELEMENT_DETAIL_PICTURE_FILE_ALT'];
                            $result[] = array(
            ...
            if ($addDetailToSlider || empty($result))
            {
                if (!empty($item['DETAIL_PICTURE']))
                {
                    ...
                    if (isset($item['DETAIL_PICTURE']['ID']))
                    {
                        ...
                        if($item['IPROPERTY_VALUES']['ELEMENT_DETAIL_PICTURE_FILE_ALT'])
                            $item['DETAIL_PICTURE']['DESCRIPTION'] = $item['IPROPERTY_VALUES']['ELEMENT_DETAIL_PICTURE_FILE_ALT'];
                        if($item['IPROPERTY_VALUES']['ELEMENT_DETAIL_PICTURE_FILE_TITLE'])
                            $item['DETAIL_PICTURE']['DESCRIPTION_TITLE'] = $item['IPROPERTY_VALUES']['ELEMENT_DETAIL_PICTURE_FILE_TITLE'];
                        array_unshift(
                            $result,
                            array(
                                ...
                                'TITLE' => ($item['DETAIL_PICTURE']['DESCRIPTION_TITLE'] ? $item['DETAIL_PICTURE']['DESCRIPTION_TITLE'] : ($item['DETAIL_PICTURE']['TITLE'] ? $item['DETAIL_PICTURE']['TITLE'] : $item['NAME'] ))
                            )
                        );
                    }
                }
...
static function __AdmSettingsSaveOption_EX($module_id, $arOption){
		...
		if($name == 'CUSTOM_FONT')
		{
			...
			if($val)
			{
				$string = str_replace('link href=', '', $val);
...
static function __AdmSettingsDrawRow_EX($module_id, $Option, $siteID, $siteDir){
		$arControllerOption = CControllerClient::GetInstalledOptions($module_id);
					...
					<div class="shops list">
						<div class="items">
							<?foreach($arShops as $arShop):?>
								<div class="item <?if($arShop["IMAGE"]) echo '';else echo 'wi';?>" data-ID="<?=$arShop['ID']?>">
									<div class="image">
										<?if($arShop["IMAGE"]):?>
