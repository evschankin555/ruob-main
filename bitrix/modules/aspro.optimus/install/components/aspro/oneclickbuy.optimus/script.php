...
else
{
	...
	$bUseCaptcha = (\Bitrix\Main\Config\Option::get('aspro.optimus', 'ONE_CLICK_BUY_CAPTCHA', 'N') == 'Y');
	...
	if(!$USER->IsAuthorized()){
		...
		if($user_registered && !$user_exists){
			...
			$fioTrimmed = trim($_POST['ONE_CLICK_BUY']['FIO']);
			if(strlen($fioTrimmed)) {
				$username = explode(' ', $fioTrimmed);
			} else if( strlen( trim($_POST['ONE_CLICK_BUY']['CONTACT_PERSON']) ) ) {
				$username = explode(' ',  trim($_POST['ONE_CLICK_BUY']['CONTACT_PERSON']));
			}
