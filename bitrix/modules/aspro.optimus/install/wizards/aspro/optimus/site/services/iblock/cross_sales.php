...
if(WIZARD_INSTALL_DEMO_DATA){
	...
	if(!$iblockID){
		$skuIBlockID = false;
		if($catalogIBlockID = COptimusCache::$arIBlocks[WIZARD_SITE_ID]['aspro_optimus_catalog']['aspro_optimus_catalog'][0]){
			if($arSku = CCatalogSKU::GetInfoByProductIBlock($catalogIBlockID)){
				$skuIBlockID = $arSku['IBLOCK_ID'];
			}

			$rakivinaSectionID = COptimusCache::CIBlockSection_GetList(
				array(
					'CACHE' => array(
						'TAG' => COptimusCache::GetIBlockCacheTag($catalogIBlockID),
						'MULTI' => 'N',
						'RESULT' => array('ID'),
					)
				),
			...
			$smesitelSectionID = COptimusCache::CIBlockSection_GetList(
				array(
					'CACHE' => array(
						'TAG' => COptimusCache::GetIBlockCacheTag($catalogIBlockID),
						'MULTI' => 'N',
			...
			$fotoSectionID = COptimusCache::CIBlockSection_GetList(
				array(
					'CACHE' => array(
						'TAG' => COptimusCache::GetIBlockCacheTag($catalogIBlockID),
						'MULTI' => 'N',
			...
			$obektivSectionID = COptimusCache::CIBlockSection_GetList(
				array(
					'CACHE' => array(
						'TAG' => COptimusCache::GetIBlockCacheTag($catalogIBlockID),
						'MULTI' => 'N',
	...
	CUserOptions::SetOption("form", "form_element_".$iblockID, array(
		"tabs" => 'edit1--#--'.GetMessage("WZD_OPTION_262").'--,--ACTIVE--#--'.GetMessage("WZD_OPTION_2").'--,--ACTIVE_FROM--#--'.GetMessage("WZD_OPTION_50").'--,--ACTIVE_TO--#--'.GetMessage("WZD_OPTION_52").'--,--NAME--#--'.GetMessage("WZD_OPTION_54").'--,--IBLOCK_ELEMENT_PROP_VALUE--#--'.GetMessage("WZD_OPTION_130").'--,--PROPERTY_'.$arProperty["SHOW_PLACE"].'--#--'.GetMessage("WZD_OPTION_254").'--,--PROPERTY_'.$arProperty["PRIORITY"].'--#--'.GetMessage("WZD_OPTION_255").'--,--PROPERTY_'.$arProperty["SORT"].'--#--'.GetMessage("WZD_OPTION_256").'--,--PROPERTY_'.$arProperty["LAST_LEVEL_RULE"].'--#--'.GetMessage("WZD_OPTION_257").'--,--PROPERTY_'.$arProperty["LAST_RULE"].'--#--'.GetMessage("WZD_OPTION_258").'--;--сedit1--#--'.GetMessage("WZD_OPTION_263").'--,--PROPERTY_'.$arProperty["PRODUCTS_FILTER"].'--#--'.GetMessage("WZD_OPTION_259").'--,--PROPERTY_'.$arProperty["EXT_PRODUCTS_FILTER"].'--#--'.GetMessage("WZD_OPTION_260").'--;--сedit2--#--'.GetMessage("WZD_OPTION_264").'--,--PROPERTY_'.$arProperty["USER_GROUPS"].'--#--'.GetMessage("WZD_OPTION_261").'--;--edit2--#--'.GetMessage("WZD_OPTION_82").'--,--SECTIONS--#--'.GetMessage("WZD_OPTION_82").'--;--',
	));
