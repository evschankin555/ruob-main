	...
	if('TYPE_1' == $arParams['TYPE_SKU'] && $arResult['OFFERS'] ){
		foreach ($arResult['OFFERS'] as $keyOffer => $arOffer)
		{
			...
			if (!empty($arOffer['DISPLAY_PROPERTIES']))
			{
				...
				foreach ($arOffer['DISPLAY_PROPERTIES'] as &$arOneProp)
				{
					...
					$arSKUProps[] = array(
						...
						'CODE' => $arOneProp['CODE'],
						'SHOW_HINTS' => $arOneProp['SHOW_HINTS'],
						'HINT' => $arOneProp['HINT'],
					);
			...
			{	
				...
				$arOffer = array_merge($arOffer, COptimus::formatPriceMatrix($arOffer));
				$sPriceMatrix = COptimus::showPriceMatrix($arOffer, $arParams, $arOffer['~CATALOG_MEASURE_NAME']);
			}
			...
			$arOneRow = array(
				...
				'ARTICLE_SKU' => ($arParams['SHOW_ARTICLE_SKU'] == 'Y' ? ((isset($arResult['PROPERTIES']['CML2_ARTICLE']['VALUE']) && $arResult['PROPERTIES']['CML2_ARTICLE']['VALUE']) ? GetMessage('ARTICLE').': '.$arResult['PROPERTIES']['CML2_ARTICLE']['VALUE'] : '') : ''),
				...
				'AVAILIABLE' => COptimus::GetQuantityArray($totalCount, array(), "Y"),
	...
	if('TYPE_1' != $arParams['TYPE_SKU'] && $arResult['OFFERS'] ){
		...
		foreach ($arResult['OFFERS'] as $keyOffer => $arOffer){
			...
			{				
				...
				$arResult['OFFERS'][$keyOffer] = array_merge($arOffer, COptimus::formatPriceMatrix($arOffer));
			}
		}
	...
	if($arParams['USE_PRICE_COUNT'] == 'Y')
	{
		if($arResult['OFFERS'])
		{
			foreach($arResult['OFFERS'] as $keyOffer => $arOffer)
			{
				...
				$arOffer["FIX_PRICE_MATRIX"] = COptimus::checkPriceRangeExt($arOffer);
				$arResult['OFFERS'][$keyOffer] = array_merge($arOffer, COptimus::formatPriceMatrix($arOffer));
			}
			$arResult['MIN_PRICE'] = COptimus::getMinPriceFromOffersExt(
				$arResult['OFFERS'],
		...
		{
			$arResult["FIX_PRICE_MATRIX"] = COptimus::checkPriceRangeExt($arResult);
		}			
	}
	
	//format prices when USE_PRICE_COUNT
	$arResult = array_merge($arResult, COptimus::formatPriceMatrix($arResult));
}
...
/*brand item*/
$arBrand = array();
$arResult["PROPERTIES"]["BRAND"]["LINK_IBLOCK_ID"] = 9;
if(strlen($arResult["PROPERTIES"]["CML2_MANUFACTURER"]["VALUE"]) && $arResult["PROPERTIES"]["BRAND"]["LINK_IBLOCK_ID"]){
	$arBrand = COptimusCache::CIBLockElement_GetList(array('CACHE' => array("MULTI" =>"N", "TAG" => COptimusCache::GetIBlockCacheTag($arResult["PROPERTIES"]["BRAND"]["LINK_IBLOCK_ID"]))), array("IBLOCK_ID" => $arResult["PROPERTIES"]["BRAND"]["LINK_IBLOCK_ID"], "ACTIVE"=>"Y", "NAME" => $arResult["PROPERTIES"]["CML2_MANUFACTURER"]["VALUE"]));
	if($arBrand){
