...
<?if($arPost["PARAMS"]):?>
	...
	if(!strlen($arPost["PARAMS"]["BASKET_URL"])){
		$arPost["PARAMS"]["BASKET_URL"] = str_replace('#SITE_DIR#', $arSite['DIR'], \Bitrix\Main\Config\Option::get('aspro.optimus', 'BASKET_PAGE_URL', '#SITE_DIR#basket/', $arPost["SITE_ID"]));
	}

	$typeSku = \Bitrix\Main\Config\Option::get('aspro.optimus', 'TYPE_SKU', 'TYPE_1', $arPost["SITE_ID"]);
	$bChangeTitleItem = \Bitrix\Main\Config\Option::get('aspro.optimus', 'CHANGE_TITLE_ITEM', 'N', $arPost["SITE_ID"]) === 'Y';
	...
	{
		$arPricesIDs = \Aspro\Functions\CAsproOptimus::getPricesID($arPost["PARAMS"]["PRICE_CODE"], true);
		if($arPricesIDs)
		{
	...
	$cachePath = "/COptimusCache/iblock/getSKUjs/".$cacheTag."/";
	$cacheTime = $arPost["PARAMS"]["CACHE_TIME"];
	// $cacheTime = 0;

	if(isset($arPost["clear_cache"]) && $arPost["clear_cache"] == "y"){
		\COptimusCache::ClearCacheByTag($cacheTag);
	}
	...
	else
	{
			...
			if($arOffer['PRICES'])
			{
				...
				if($arPost["PARAMS"]["USE_PRICE_COUNT"] == "Y")
				{
					if(function_exists('CatalogGetPriceTableEx'))
					{
						...
						{
							$arOffer = array_merge($arResult["ITEMS"][$key], COptimus::formatPriceMatrix($arResult["ITEMS"][$key]));
		...
		foreach($arResult["ITEMS"] as $key => $arItem)
		{
			$arItems["ITEMS"][$key] = array(
				...
				"TOTAL_COUNT" => COptimus::GetTotalCount($arItem, $arPost["PARAMS"]),
	...
	if($arItems)
	{
		foreach($arItems["ITEMS"] as $key => $arItem)
		{
			...
			$arAddToBasketData = COptimus::GetAddToBasketArray($arItem, $arItem["TOTAL_COUNT"], $arPost["PARAMS"]["DEFAULT_COUNT"], $arPost["PARAMS"]["BASKET_URL"], false, array(), 'small', $arPost["PARAMS"]);
			...
			$arItems["ITEMS"][$key]["PRICES_HTML"] = \Aspro\Functions\CAsproOptimusItem::showItemPrices($arPost["PARAMS"], $arItem["PRICES"], $arItem['MEASURE'], $arItem["MIN_PRICE"]["ID"], ($arPost["PARAMS"]['SHOW_DISCOUNT_PERCENT_NUMBER'] == "Y" ? "N" : "Y"), false, true);
			...
			$arItems["ITEMS"][$key]["AVAILIABLE"] = COptimus::GetQuantityArray($arItem["TOTAL_COUNT"]);
			...
			// 	$arItems["ITEMS"][$key]["GALLERY"] = COptimus::getSliderForItemExt($arItem, $arPost["PARAMS"]["OFFER_ADD_PICT_PROP"], true);

			// 	// if(!$arItems["ITEMS"][$key]["GALLERY"] && $arItems['MAIN_ITEM'])
			// 	if($arItems['MAIN_ITEM'] && 'Y' == $arPost["PARAMS"]['ADD_DETAIL_TO_SLIDER'])
			// 	{
			// 		$arItems["ITEMS"][$key]["GALLERY"] = array_merge($arItems["ITEMS"][$key]["GALLERY"], COptimus::getSliderForItemExt($arItems['MAIN_ITEM'], $arPost["PARAMS"]["ADD_PICT_PROP"], 'Y' == $arPost["PARAMS"]['ADD_DETAIL_TO_SLIDER']));
			// 	}

			// 	array_splice($arItems["ITEMS"][$key]["GALLERY"], $arPost["PARAMS"]["MAX_GALLERY_ITEMS"]);
			// 	$arItems["ITEMS"][$key]["GALLERY_HTML"] = \Aspro\Functions\CAsproOptimusItem::showSectionGallery( array('ITEM' => $arItems["ITEMS"][$key], 'RETURN' => true) );
			// }
			...
			$arItems["ITEMS"][$key]["ONE_CLICK_BUY_HTML"] = \Aspro\Functions\CAsproOptimus::showItemOCB($arAddToBasketData, $arItem, $arPost["PARAMS"], true);
			...
			{
				$arItems["ITEMS"][$key]["PRICE_MATRIX_HTML"] = COptimus::showPriceMatrix($arItem, $arPost["PARAMS"], $arItem['MEASURE']);
				...
				// $arItems["ITEMS"][$key]["PRICE_MATRIX_TOP_HTML"] = COptimus::showPriceRangeTop($arItem, $arPost["PARAMS"], GetMessage("CATALOG_ECONOMY"));
			}
	...
	<script>
		...
		setBuyBlock = function(th, obj, index)
		{
			...
			if((obj.CONFIG.OPTIONS.USE_PRODUCT_QUANTITY_LIST && obj.CONFIG.ACTION == "ADD") && obj.CAN_BUY)
			{
				var max=(obj.CONFIG.MAX_QUANTITY_BUY>0 ? "data-max='"+obj.CONFIG.MAX_QUANTITY_BUY+"'" : ""),
					counterHtml='<span class="minus dark-color"><?=\COptimus::showIconSvg("wish ncolor colored1", SITE_TEMPLATE_PATH."/images/svg/minus.svg");?></span>'+
						'<input type="text" class="text" name="'+obj.PRODUCT_QUANTITY_VARIABLE+'" value="'+input_value+'" />'+
						'<span class="plus dark-color" '+max+'><?=\COptimus::showIconSvg("wish ncolor colored1", SITE_TEMPLATE_PATH."/images/svg/plus.svg");?></span>';
			...
			if(obj.CAN_BUY)
			{
				th.find('.like_icons.block .wrapp_one_click').html('<span class="rounded2 colored_theme_hover_bg" data-item="'+obj.ID+'" data-iblockID="'+obj.IBLOCK_ID+'" data-quantity="'+obj.CONFIG.MIN_QUANTITY_BUY+'" onclick="oneClickBuy(\''+obj.ID+'\', \''+obj.IBLOCK_ID+'\', this)" title="<?=GetMessage('ONE_CLICK_BUY')?>">'+
					'<?=\COptimus::showIconSvg("fw ncolor colored", SITE_TEMPLATE_PATH."/images/svg/quickbuy.svg");?>'+
					'</span>');
			}
		...
		setPriceAction = function(obj, sku, change)
		{
			...
			if(arOptimusOptions['THEME']['SHOW_TOTAL_SUMM'] == 'Y')
			{
				...
				// if(arOptimusOptions["THEME"]["SHOW_TOTAL_SUMM_TYPE"] == "ALWAYS")
		...
		setPriceMatrix = function(sPriceMatrix, obj, currentPriceSelected)
		{
			...
			if (wrapper.find('.cost').length)
			{
				...
				if(obj.SHOW_POPUP_PRICE)
					wrapper.find('.cost .js_price_wrapper').append('<div class="js-show-info-block more-item-info rounded3 bordered-block text-center"><?=\COptimus::showIconSvg("fw", SITE_TEMPLATE_PATH."/images/svg/dots.svg");?></div>');
		...
		ChangeInfo = function()
		{
			...
			if(-1 < index)
			{
				...
				if(arOptimusOptions["THEME"]["TYPE_SKU"] === "TYPE_1" && arOptimusOptions["THEME"]["CHANGE_TITLE_ITEM"] === "Y"){
