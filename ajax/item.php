	...
	if($_REQUEST["subscribe_item"] == "Y"){
		if(class_exists('\Bitrix\Catalog\Product\SubscribeManager')){
			...
			if($itemID && ($bSubscribeProducts || $userId))
			{
				...
				$arSubscribeList = COptimus::getUserSubscribeList($userId);
...
elseif(!empty($_REQUEST["compare_item"])){
    ...
    if (
        ...
        !empty($_SESSION["CATALOG_COMPARE_LIST"][$iblock_id]) &&
        is_array($_SESSION["CATALOG_COMPARE_LIST"][$iblock_id]["ITEMS"]) &&
        array_key_exists($_REQUEST["item"], $_SESSION["CATALOG_COMPARE_LIST"][$iblock_id]["ITEMS"])
    ) {
        unset($_SESSION["CATALOG_COMPARE_LIST"][$iblock_id]["ITEMS"][$_REQUEST["item"]]);
    } else {
        // Также обрабатываем случай, если элемента нет в списке сравнения
        $elementInfo = CIBlockElement::GetByID($_REQUEST["item"])->Fetch();

        // Проверяем, чтобы $elementInfo был не пустым, прежде чем добавлять его в список
        if (!empty($elementInfo)) {
            $_SESSION["CATALOG_COMPARE_LIST"][$iblock_id]["ITEMS"][$_REQUEST["item"]] = $elementInfo;
        }
    }
}
